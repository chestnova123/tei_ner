...
# version 4.6.0
# Changes from 4.5.0:
#   - Marginal insertion integration: <metamark function="Einfügung" target="...">
#     elements in main paragraphs are replaced inline by the character stream of
#     the target margin paragraph at the exact position of the metamark.
#   - build_insertion_map() validates all insertion targets across the registry:
#     reports duplicates (same paragraph targeted more than once) and skips them.
#   - Margin paragraphs that have been integrated as insertions are excluded from
#     standalone corpus processing.
#   - Insertion paragraph entities are included in entity extraction.
#   - <ptr> return-pointer elements inside margin paragraphs are silently ignored.
# Changes from 4.4.0:
#   - added pack examples
...
# ============================================================
# 2b. Marginal insertion map
# ============================================================

def build_insertion_map(registry: dict) -> tuple[dict, set]:
    """
    Scan every registered paragraph for <metamark function="Einfügung" target="...">
    elements and build a validated insertion map.

    Returns:
        insertion_map : dict mapping
            (host_fname_lower, host_p_id) ->
                list of (metamark_elem, target_key)
            where target_key is (fname_lower, p_id) of the margin paragraph.
            The list preserves document order so multiple insertions in one
            paragraph are applied in the correct sequence.

        inserted_p_ids : set of (fname_lower, p_id) keys that have been
            claimed as insertion targets. These must be excluded from
            standalone paragraph processing.

    Validation rules (per your spec):
        - A margin paragraph targeted more than once is an error: reported and
          skipped (all occurrences of that target are removed from the map).
        - A target that does not exist in the registry is reported and skipped.
    """
    XML_NS = "http://www.w3.org/XML/1998/namespace"

    # First pass: collect all (host_key, metamark_elem, target_key) triples
    raw = []   # list of (host_key, metamark_elem, target_key)
    target_count = Counter()   # target_key -> how many times it is referenced

    for host_key, entry in registry.items():
        p_elem      = entry["elem"]
        fname_lower = entry["filename"].lower()

        # Find all metamark elements with function="Einfügung" and a target
        for mm in p_elem.xpath(
            ".//*[local-name()='metamark'"
            "     and @function='Einfügung'"
            "     and @target]"
        ):
            raw_target = mm.get("target", "").strip()
            if not raw_target:
                continue

            target_key = _normalise_ref(raw_target, fname_lower)
            raw.append((host_key, mm, target_key))
            target_count[target_key] += 1

    # Second pass: validate and build the map
    duplicate_targets = {k for k, n in target_count.items() if n > 1}
    missing_targets   = set()

    if duplicate_targets:
        print(f"\n[INSERTION WARNING] {len(duplicate_targets)} margin paragraph(s) "
              f"are targeted more than once — skipping all their insertions:")
        for k in sorted(duplicate_targets):
            print(f"  {k[0]}#{k[1]}  (referenced {target_count[k]} times)")

    # Build host_key -> ordered list of (metamark_elem, target_key)
    insertion_map: dict = defaultdict(list)
    inserted_p_ids: set = set()

    for host_key, mm, target_key in raw:
        if target_key in duplicate_targets:
            continue
        if target_key not in registry:
            if target_key not in missing_targets:
                print(f"  [INSERTION WARNING] target {target_key[0]}#{target_key[1]} "
                      f"not found in registry — skipping")
                missing_targets.add(target_key)
            continue
        insertion_map[host_key].append((mm, target_key))
        inserted_p_ids.add(target_key)

    total_insertions = sum(len(v) for v in insertion_map.values())
    print(f"\n[INSERTION] Built insertion map: "
          f"{total_insertions} valid insertions across "
          f"{len(insertion_map)} host paragraphs. "
          f"{len(inserted_p_ids)} margin paragraphs will be excluded "
          f"from standalone processing.")

    return dict(insertion_map), inserted_p_ids
...
def normalize_paragraph_raw(p_elem, insertion_map=None, registry=None,
                             host_key=None):
    """
    Build (text_chars, char_map, insertion_p_elems) from a <p> element
    WITHOUT calling postprocess_hyphenation.

    If insertion_map is provided and this paragraph has insertion targets,
    each <metamark function="Einfügung" target="..."> is replaced inline
    by the character stream of the target margin paragraph.

    Returns:
        text_chars        : list of characters
        char_map          : parallel list of (node, kind, offset) tuples
        insertion_p_elems : list of lxml <p> elements from margin paragraphs
                            that were inlined (needed for entity extraction)
    """
    text_chars, char_map = [], []
    insertion_p_elems    = []

    # Build a fast lookup: metamark element id -> target registry entry
    # keyed by the Python id() of the metamark lxml element
    mm_to_target: dict = {}
    if insertion_map and host_key and host_key in insertion_map:
        for mm_elem, target_key in insertion_map[host_key]:
            if registry and target_key in registry:
                mm_to_target[id(mm_elem)] = registry[target_key]
    ...
    def recurse(node):
        ...
        for child in list(node):
            ...
            if tag == "lb":
                ...
                func = (child.get("function") or "").strip()
                if func == "Einfügung" and id(child) in mm_to_target:
                    # Inline the margin paragraph character stream here
                    target_entry = mm_to_target[id(child)]
                    target_p     = target_entry["elem"]
                    insertion_p_elems.append(target_p)

                    # Recursively normalise the margin paragraph using a
                    # dedicated call that ignores further insertion markers
                    # (margin paragraphs do not themselves contain insertions)
                    ins_chars, ins_map, _ = normalize_paragraph_raw(
                        target_p,
                        insertion_map=None,  # no nested insertions
                        registry=None,
                        host_key=None,
                    )

                    # Insert a space before the insertion so it does not
                    # run into the preceding word
                    if text_chars and text_chars[-1] not in (" ", "\t", "\n"):
                        append_literal(" ", child, "marker")

                    text_chars.extend(ins_chars)
                    char_map.extend(ins_map)

                    # Insert a space after the insertion
                    if ins_chars and ins_chars[-1] not in (" ", "\t", "\n"):
                        append_literal(" ", child, "marker")
                else:
                    # Non-insertion metamark or unresolved target — skip
                    pass

            elif tag == "ptr":
                # Return pointers inside margin paragraphs — silently ignored
                pass
    ...
    recurse(p_elem)
    return text_chars, char_map, insertion_p_elems
...
def load_all_paragraphs(xml_files: list[str]) -> dict:
    ...
    XML_NS = "http://www.w3.org/XML/1998/namespace"
    _diag_done = False  # print attribute diagnostic once only
    ...
    for xml_path in xml_files:
        ...
        # One-time diagnostic: show actual attribute keys on first <p> with an id
        if not _diag_done and p_elems:
            for _p in p_elems:
                if any("id" in k.lower() for k in _p.attrib):
                    print(f"  [DIAG] First <p> attribs: {dict(_p.attrib)}")
                    _diag_done = True
                    break

        for p in p_elems:
            # xml:id is a namespaced attribute in lxml:
            # {http://www.w3.org/XML/1998/namespace}id
            p_id = (
                p.get(f"{{{XML_NS}}}id")   # correct: {xml namespace}id
                or p.get("xml:id")          # fallback for non-namespace-aware parsers
                or p.get("id")              # bare fallback
            )
            if not p_id:
                continue
...
def materialise_logical_paragraph(lp: dict, registry: dict,
                                   insertion_map: dict = None) -> tuple:
    """
    Given a logical paragraph (one or two fragment keys), produce the
    final (text, char_map, p_elems) by:
      1. Calling normalize_paragraph_raw on each fragment, inlining any
         marginal insertions at their metamark positions.
    ...
    p_elems returned includes both the main paragraph elements and any
    margin paragraph elements that were inlined, so that entity extraction
    covers entities in the inserted text.
    ...
    for idx, key in enumerate(lp["keys"]):
        ...
        chars, cmap, ins_p_elems = normalize_paragraph_raw(
            p_elem,
            insertion_map=insertion_map,
            registry=registry,
            host_key=key,
        )
        # Include inlined margin paragraph elements for entity extraction
        p_elems.extend(ins_p_elems)

        if idx > 0:
            sentinel_node = p_elems[0]
            all_chars.append(" ")
...
def build_examples_from_chains(
    ...
    file_to_family:  dict,
    file_to_split:   dict,
    insertion_map:   dict = None,
    inserted_p_ids:  set  = None,
) -> list[dict]:
    """
    For each chain -> logical paragraphs -> materialise -> BIO chunk -> examples.

    Paragraphs that are insertion targets (inserted_p_ids) are skipped when
    encountered as standalone chain members — they have already been inlined
    into their host paragraphs.
    """
    if insertion_map   is None: insertion_map   = {}
    if inserted_p_ids  is None: inserted_p_ids  = set()
    ...
    for chain in chains:
        ...
        # Skip chains whose sole member is an insertion target — it has
        # already been inlined into the host paragraph
        if len(chain) == 1 and chain[0] in inserted_p_ids:
            continue
        ...
        for lp_idx, lp in enumerate(logical_paragraphs):
            # Skip any logical paragraph whose keys are all insertion targets
            if all(k in inserted_p_ids for k in lp["keys"]):
                continue

            result = materialise_logical_paragraph(
                lp, registry, insertion_map=insertion_map
            )
            if result is None:
                continue
...
def load_corpus(root_dir: str, csv_path: str) -> list[dict]:
    ...
    Full pipeline:
      ...
      5. Build insertion map from <metamark function="Einfügung"> elements
      6. Build chains from prev/next links
      7. Build examples (insertion targets excluded from standalone processing)
    """
    ...
    # Build insertion map before chains so inserted_p_ids can filter chains
    insertion_map, inserted_p_ids = build_insertion_map(registry)
